...
struct data_s {
  ...
  // gpu_aware tag stored at entry time (pointer is live); keyed by
  // {hostname, vpid, vtid} and consumed at the matching exit.
  std::unordered_map<thapi_call_key_t, int8_t, thapi_call_key_hash_t> gpu_aware_state;
};
...
static void send_host_message(void *btx_handle,
  ...
  const thapi_call_key_t key{hostname, vpid, vtid};
  auto &gpu_map = static_cast<data_t *>(usr_data)->gpu_aware_state;
  auto it = gpu_map.find(key);
  int8_t gpu_aware = (it != gpu_map.end()) ? it->second : 0;
  if (it != gpu_map.end())
    gpu_map.erase(it);
...
static void entries_buf_ptr_callback(void *btx_handle,
                                     void *usr_data,
                                     int64_t ts,
                                     const char *event_class_name,
                                     const char *hostname,
                                     ...
                                     uint64_t vtid,
                                     const void *buf) {
  // entries_callback already recorded the timestamp; just tag gpu_aware here.
  const int8_t tag = (int8_t)thapi_is_gpu_aware_ptr(buf);
  static_cast<data_t *>(usr_data)->gpu_aware_state[{hostname, vpid, vtid}] = tag;
}
...
void btx_register_usr_callbacks(void *btx_handle) {
  ...
  btx_register_callbacks_entries_buf_ptr(btx_handle, &entries_buf_ptr_callback);
