...
/* GPU-aware pointer classification (L0 / CUDA / HIP) */
extern "C" {
#include "tracer_mpi_helpers.include.c"
}

struct data_s {
  EntryState entry_state;
  /* Per-thread entry-side gpu_aware flag: keyed by {hostname, vpid, vtid}.
   * Set at entry_callback, consumed at exit_callback to emit the tag. */
  std::unordered_map<thapi_call_key_t, int8_t> gpu_aware_state;
};
...
static void send_host_message(void *btx_handle,
  ...
  data_t *d = static_cast<data_t *>(usr_data);
  std::string event_class_name_striped = strip_event_class_name_exit(event_class_name);
  const int64_t entry_ts =
      d->entry_state.get_ts({hostname, vpid, vtid});

  /* Retrieve and clear the gpu_aware flag recorded at entry time */
  int8_t gpu_aware = 0;
  auto it = d->gpu_aware_state.find({hostname, vpid, vtid});
  if (it != d->gpu_aware_state.end()) {
    gpu_aware = it->second;
    d->gpu_aware_state.erase(it);
  }
...
/* Generic entry: record timestamp only (no buffer pointer available) */
...
/* GPU-aware entry: record timestamp AND classify the primary buffer pointer.
 * Called for any MPI function that carries a sendbuf / recvbuf / buf argument
 * (generated by gen_mpi_library.rb for functions tagged :has_buf in
 * mpi_meta_parameters.yaml).  We classify the first non-NULL, non-MPI_IN_PLACE
 * buffer argument; the caller picks the most informative one (sendbuf > recvbuf). */
static void entries_gpu_aware_callback(void *btx_handle,
                                       void *usr_data,
                                       int64_t ts,
                                       const char *event_class_name,
                                       const char *hostname,
                                       ...
                                       const void *buf) {
  data_t *d = static_cast<data_t *>(usr_data);
  d->entry_state.set_ts({hostname, vpid, vtid}, ts);
  d->gpu_aware_state[{hostname, vpid, vtid}] =
      (int8_t)thapi_is_gpu_aware_ptr(buf);
}
...
void btx_register_usr_callbacks(void *btx_handle) {
  ...
  /* Generic entry (no buffer pointer in the event) */
  btx_register_callbacks_entries(btx_handle, &entries_callback);
  /* GPU-aware entry (buffer pointer available – classify at entry time) */
  btx_register_callbacks_entries_gpu_aware(btx_handle, &entries_gpu_aware_callback);
